...
const F = {
  ...
  BwdQueued: 64,
  /** Source-only: a deferred backward write targets this source; reading it
   *  (or `_update`) must resolve the queue first. Folded into the source
   *  getter's existing flag test, so UNflagged reads pay nothing. */
  BwdPending: 128,
} as const;
 ...
 *  source-read drain and `flush`. */
let bwdDrained = 0;
/** Re-entrancy guard: a resolve walk reads cells internally. */
let draining = false;
...
export class Cell<T = unknown> implements ReactiveNode {
  ...
  _enqueueBwd(): void {
    ...
    // and the same slot resolves last-write-wins.
  ...
  _update(): boolean {
    ...
    if (this.flags & F.BwdPending) drainPending(); // resolve before checkDirty commits
...
Object.defineProperty(Cell.prototype, "value", {
  get(this: Cell<unknown>): unknown {
    ...
    // Cell path. The widened mask costs the same single bit-test as the
    // old `flags & F.Dirty` — an UNflagged source pays nothing for the
    // backward gate (the whole point of folding it into the flags).
    if (flags & (F.Dirty | F.BwdPending)) {
      if (flags & F.BwdPending) drainPending(); // pull: resolve deferred view writes → sets pendingValue + Dirty
      if (this.flags & F.Dirty) {
        this.flags = F.Mutable; // clears Dirty + BwdPending
        const prevV = this.currentValue;
        this.currentValue = this.pendingValue;
  ...
  set(this: Cell<unknown>, next: unknown): void {
    ...
    if (deferred) {
      if (Array.isArray(b.parent) || b.stateful !== undefined) {
        // Multi-parent / stateful: existing deferred path, resolved at
        // flush (mid-batch reads of these are stale, as before).
        if (this._equals(next, this.peek())) return;
        this.pendingValue = next;
        this._enqueueBwd();
        return;
      }
      // Single-parent lens: PUSH. Stash the target + enqueue, then mark the
      // ROOT SOURCE BwdPending and push forward invalidation NOW (cheap,
      // flag-only, no `put`). The put-chain is PULLED lazily when the
      // source (or a dependent) is read or at flush. Repeated writes
      // overwrite `pendingValue` and skip the re-mark — coalesced. Net-zero
      // resolves to a no-op source write, so nothing over-fires.
      const firstQueue = !(this.flags & F.BwdQueued);
      this.pendingValue = next;
      this._enqueueBwd();
      if (firstQueue) markBwdPending(this);
      return;
    }
...
/** Resolve every not-yet-drained backward write (each queued view → its
 *  source via the put-chain, once, last-write-wins). Triggered by a read of
 *  a `BwdPending` source and by `flush`. The `draining` guard makes the
 *  internal reads of a resolve walk no-op the drain instead of recursing. */
function drainPending(): void {
  ...
  try {
    while (bwdDrained < bwdQueue.length) {
      ...
      const cb = cell._bwd!;
      // Stale-slot skip only (last-write supersession). NOT gated on
      // BwdQueued: a view read mid-batch enters `_update`, which overwrites
      // its flags (clearing BwdQueued) before this resolves it — the cursor
      // + queueIdx already guarantee once-per-slot resolution.
      if (cb.queueIdx !== bwdDrained) {
        bwdDrained++;
        continue;
      }
      bwdDrained++;
...
/** Mark a single-parent view's root source `BwdPending` and push forward
 *  invalidation, so reads resolve the deferred put-chain lazily. Walks the
 *  chain flag-only (no `put`); bails (leaving the write flush-resolved) if
 *  the chain hits a pin / multi-parent / merge / stateful boundary. */
function markBwdPending(view: Cell<unknown>): void {
  let s = view;
  while (s.getter !== undefined) {
    const sb = s._bwd;
    if (sb === undefined) return;
    const p = sb.parent;
    if (p === undefined || Array.isArray(p)) return;
    s = p;
  }
  if (!(s.flags & F.BwdPending)) {
    s.flags |= F.Dirty | F.BwdPending;
    const subs = s.subs;
    if (subs !== undefined) propagate(subs, runDepth > 0, activeNetwork);
  }
}
