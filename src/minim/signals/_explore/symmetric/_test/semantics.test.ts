...
// down the two equality checks and the consequences for lossy lenses,
// where this engine deliberately differs from the old fused engine.
//
// Two equality checks, dual to each other:
//   * SOURCE check (`_writeSource`): a backward write that resolves to
//     the source's current value propagates nothing forward.
//   * VIEW check (lens setter): writing a lens the value it already
...
import { computed, effect, iso, signal } from "../index";

const quantize = (step: number) => (v: number) => Math.round(v / step) * step;

describe("view check: writing the displayed value is a no-op", () => {
  it("quantize: misaligned source does NOT jump to the grid", () => {
    const root = signal(7);
    const q = iso(root, quantize(10), quantize(10));
    ...
    // Here the view check makes this a no-op: root stays misaligned.
...
describe("view check trustworthiness (clean cache only)", () => {
  it("write before any read still cascades (cache not yet valid)", () => {
    const root = signal(7);
    const q = iso(root, quantize(10), quantize(10));
    // No read of q first → cache stale → check cannot fire → cascades.
    q.value = 10;
    expect(root.value).toBe(10); // put(10)=10 committed
  });

  it("a pending (upstream-changed) lens cascades, not absorbed", () => {
    const root = signal(7);
    const id = iso(
      root,
      (v) => v,
    ...
    id.value = 7; // equals the STALE cache — must NOT be absorbed
    expect(root.value).toBe(7); // genuine edit back to 7 committed
    expect(id.value).toBe(7);
  });
});

describe("source check: backward write resolving to current source", () => {
  it("identity chain: writing the current source value propagates nothing", () => {
    ...
    a.value = 6; // displayed value → view check no-op (root untouched)
    expect(fn).toHaveBeenCalledTimes(1);
