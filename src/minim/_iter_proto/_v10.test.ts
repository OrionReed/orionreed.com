...
import "../_test/setup";
import { Anim, TracedAnim, sleep, drive, ignoreErrors, fromEvent, type Animator, type SuspendFn } from "./v10_min";

describe("v10 / yield contract", () => {
  ...
  it("sleep N sec via userland accumulator", () => {
    const a = new Anim();
    let woke = false;
    a.start(function* () { yield* sleep(0.1); woke = true; });
  ...
  it("yield N is rejected (no more engine sleep)", () => {
    const a = new Anim();
    a.onError = () => {};
    let captured: any;
    a.start(function* () { yield 0.5 as any; }, (_v, err) => { captured = err; });
    a.step(0);
    expect((captured as Error)?.message).toMatch(/unsupported/);
  });
});

describe("v10 / TracedAnim", () => {
