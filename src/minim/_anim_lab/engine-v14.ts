// v14 — v13 with a smaller decoupling pattern.
//
// Instead of "engine is pure stepping + external `attachRaf` adapter +
// `onActivity` slot + `hasWork()` predicate", inject the scheduler at
// construction:
//
//   new Anim()                              // browser: auto-rAF
//   new Anim({ schedule: () => 0 })          // headless: no-op
//   new Anim({ schedule: setTimeout-style }) // any custom driver
//
// The engine still owns its `kick()` — the only concept exposed.
// Callers don't need to remember to attach anything. Diagrams just
// `new Anim()`. Tests pass a no-op explicitly.
//
// Saves ~25 lines vs v13 (no attachRaf, no onActivity, no hasWork)
// and removes 3 user-visible concepts.
...
export class Anim implements RuntimeAccess {
  ...
  /** Fire `cb(dt)` once per `step(dt)`. Returns a disposer. The engine
   *  doesn't track absolute time; consumers accumulate from deltas. */
