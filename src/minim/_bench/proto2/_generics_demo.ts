...
section("mean<number> (via Num's algebra)");
{
  const a = Num(0);
  const b = Num(10);
  const c = Num(20);
...
section("mean<Vec> (via Vec's algebra — same generic mean function)");
{
  const a = Vec({ x: 0, y: 0 });
...
// ── springStep — generic over any algebra+metric type ──────────────
...
// ── Inline new type with capabilities — no defineStruct ceremony ───

section("Inline new type works out of the box");
{
  // Define an Angle type — radians, with algebra + lerp + metric.
  interface Angle { rad: number }
  const Angle = defineType<Angle>({
    name: "Angle",
    defaults: { rad: 0 },
