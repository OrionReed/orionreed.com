...
describe("AVBD sketchpad — mixed constraint types at scale", () => {
  it("100 points, 200 mixed constraints, drag one point", () => {
    // Build a "drawing": a grid of points with horizontal /
    // vertical / diagonal constraints. Drag a corner. Time per
    // step.
    const N = 10;
    const cells: Cell[][] = [];
    for (let j = 0; j < N; j++) {
      const row: Cell[] = [];
      for (let i = 0; i < N; i++) row.push(new Cell(2, [i, j]));
      cells.push(row);
    }
    cells[0]![0]!.mass = 0;
    const s = new Solver({ iterations: 8 });
    for (const row of cells) for (const c of row) s.addCell(c);
    // Distance constraints horizontally + vertically (≈ 180 of them).
    for (let j = 0; j < N; j++) {
    ...
    // Add parallel constraints for diagonals (≈ 20).
    for (let i = 0; i < N - 1; i++) {
      parallel(s, cells[0]![i]!, cells[0]![i + 1]!, cells[N - 1]![i]!, cells[N - 1]![i + 1]!);
    }
    // Drag one corner; let everything settle.
    cells[N - 1]![N - 1]!.mass = 0;
    cells[N - 1]![N - 1]!.position[0]! = N - 1 + 0.5;
    cells[N - 1]![N - 1]!.position[1]! = N - 1 + 0.5;
    ...
    for (let i = 0; i < drags; i++) {
      dy += 0.05;
      cells[N - 1]![N - 1]!.position[1]! = N - 1 + dy;
      s.step();
    }
  ...
  it("1000 points, ~2500 mixed constraints (CAD-scale)", () => {
    // 1000 points arranged as 32×32 grid (1024 cells). On top:
    //   - distance constraints between neighbours
    //   - parallel constraints on rows
    //   - some on-circle constraints binding subsets
    //   - perpendicular constraints for "right angle" features
    const N = 32;
    const cells: Cell[][] = [];
    for (let j = 0; j < N; j++) {
      const row: Cell[] = [];
    ...
    // Distance: horizontal + vertical neighbours (≈ 1984 constraints).
    for (let j = 0; j < N; j++)
      for (let i = 1; i < N; i++)
    ...
    // Parallel: every other row's first segment parallel to the row above.
    for (let j = 1; j < N; j += 2) {
      parallel(
        s,
        cells[j]![0]!,
    ...
    // Perpendicular: a few "right angle" markers between rows.
    for (let j = 0; j < N; j += 4) {
      perpendicular(
        s,
        cells[j]![0]!,
    ...
    // Some on-circle binding to fixed centers.
    const center = new Cell(2, [N / 2, N / 2]);
    center.mass = 0;
    s.addCell(center);
    for (let j = 0; j < N; j += 8) {
      for (let i = 0; i < N; i += 8) {
  ...
  it("synthetic CAD: 200 points + bracket-like structure", () => {
    // A bracket: rectangle with internal constraints fixing aspect
    // ratio + diagonal alignment. 200 points connected.
    const N = 200;
    const cells: Cell[] = [];
    for (let i = 0; i < N; i++) {
      const angle = (i / N) * 2 * Math.PI;
      cells.push(new Cell(2, [Math.cos(angle), Math.sin(angle)]));
    }
    cells[0]!.mass = 0;
    const s = new Solver({ iterations: 8 });
    for (const c of cells) s.addCell(c);

    // Polygon: distances between consecutive vertices.
    const target = (2 * Math.PI) / N; // small chord length for unit circle
    for (let i = 1; i < N; i++) distance(s, cells[i - 1]!, cells[i]!, target);
    distance(s, cells[N - 1]!, cells[0]!, target);

    // On-circle: every 10th vertex pinned to unit circle.
    const center = new Cell(2, [0, 0]);
    center.mass = 0;
    s.addCell(center);
    for (let i = 0; i < N; i += 10) onCircle(s, cells[i]!, center, 1);

    // Drag one vertex slightly.
    cells[N / 2]!.mass = 0;
    cells[N / 2]!.position[0]! += 0.1;
    for (let i = 0; i < 3; i++) s.step();
    const drags = 20;
    const t0 = performance.now();
    for (let i = 0; i < drags; i++) {
      cells[N / 2]!.position[0]! += 0.001;
      s.step();
    }
    const t = (performance.now() - t0) / drags;
    console.log(`  200-vertex polygon + circle constraints: ${t.toFixed(2)}ms/step`);
    // Verify all vertices stay close to the unit circle (within reason).
    let maxR = 0;
    for (let i = 0; i < N; i += 10) {
      const r = Math.hypot(cells[i]!.position[0]!, cells[i]!.position[1]!);
...
describe("AVBD sketchpad — does FD overhead matter?", () => {
  it("handcoded distance vs FD-based distance, same result", () => {
    // Build the same chain with handcoded distance and FD generic;
    // verify they converge to the same answer.
    const N = 16;

    // Handcoded.
    const hcCells: Cell[] = [];
    for (let i = 0; i < N; i++) hcCells.push(new Cell(2, [i, 0]));
    hcCells[0]!.mass = 0;
    ...
    hcCells[N - 1]!.mass = 0;
    hcCells[N - 1]!.position[0]! = N - 4;
    hcCells[N - 1]!.position[1]! = 2;
    for (let i = 0; i < 10; i++) hcS.step();

    // FD-based.
    const fdCells: Cell[] = [];
    for (let i = 0; i < N; i++) fdCells.push(new Cell(2, [i, 0]));
    fdCells[0]!.mass = 0;
    ...
    fdCells[N - 1]!.mass = 0;
    fdCells[N - 1]!.position[0]! = N - 4;
    fdCells[N - 1]!.position[1]! = 2;
    for (let i = 0; i < 10; i++) fdS.step();

    // Compare configurations — they may differ in fold direction
    // (under-determined), but distances should match.
    for (let i = 1; i < N; i++) {
      const dHc = Math.hypot(
        hcCells[i]!.position[0]! - hcCells[i - 1]!.position[0]!,
        hcCells[i]!.position[1]! - hcCells[i - 1]!.position[1]!,
      );
      const dFd = Math.hypot(
        fdCells[i]!.position[0]! - fdCells[i - 1]!.position[0]!,
        fdCells[i]!.position[1]! - fdCells[i - 1]!.position[1]!,
      );
