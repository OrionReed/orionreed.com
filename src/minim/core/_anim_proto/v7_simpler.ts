// v7: drastic protocol simplification, building on v6.
//
// VERDICT after benching: this design has appealing ergonomics on
// paper but a real ~2x perf regression on suspend-heavy workloads
// (`ui-buttons`). Root cause: adding tagged-object yieldables
// (RaceSpec / FrameSpec) forces `advance()`'s hot path to do property
// probes (`.race`, `.frame`, `.then`) on every yielded Generator —
// which makes the ICs polymorphic for `Generator | Promise | Race |
// Frame` and deoptimises the most-common yield path.
//
// v7b confirms class-based markers (`instanceof RaceSpec`) help only
// marginally — the cost is structural. Function-typed yields
// (`SuspendFn` as the only "engine call") are faster precisely
// because `typeof v === 'function'` is monomorphic.
//
// Lesson: the 3-arg `SuspendFn(wake, spawn, anim)` "bureaucracy" is
// imaginary — 9 of 13 production callsites already write `(wake)`
// and ignore the extras. There's no real ergonomic gain to amputate.
// Recommendation: stay with v5/v6, do not adopt v7.
//
...
export interface RaceSpec { readonly race: readonly Yieldable[]; }

/** Per-frame callback; completes when `cb` returns `false`. */
export interface FrameSpec { readonly frame: (dt: number, t: number) => boolean | void; }
...
/** Race convenience constructor. Equivalent to `yield { race: kids }`. */
export const race = <Cs extends readonly Yieldable[]>(...kids: Cs): RaceSpec =>
  ({ race: kids });

/** Frame-driven loop. Equivalent to `yield { frame: cb }`. */
...
const isRace = (v: any): v is RaceSpec =>
  typeof v === "object" && v !== null && Array.isArray((v as any).race);

const isFrame = (v: any): v is FrameSpec =>
  typeof v === "object" && v !== null && typeof (v as any).frame === "function";
  ...
  private advance(a: Active, resume: any, asThrow = false): void {
    ...
    try {
      ...
      while (!r.done) {
        ...
        if (isRace(v)) return this.spawnRace(a, v.race);
        if (isFrame(v)) return this.subscribeFrame(a, v.frame);
