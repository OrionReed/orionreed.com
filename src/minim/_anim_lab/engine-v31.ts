// v31 — mini + `spawn` arg in SuspendFn.
//
// Time-scale is OUT of the engine: use the `mapDt(fn, gen)` userland
// combinator. The runtime knows nothing about scale, so the hot path
// is a single `wakeAt <= clock` check per active.
//
// Restored from mini: `spawn(child, onComplete)` arg on SuspendFn —
// needed for race / all / suspension-spawning combinators. Sleeps run
// against the engine clock; tickers run unscaled. drive() registers
// an onFrame ticker; per-frame `gen.next()` is bypassed.
...
class Active {
  // wakeAt: 0 ready · >0 sleep target (engine clock) · Infinity parked.
  wakeAt = 0;
...
export class Anim implements RuntimeAccess {
  ...
  // it changed within the frame. Steady state pays no compaction work.
  private deadSeen = 0;
  clock = 0;
